...
export const DailyActivityChart: React.FC<DailyActivityChartProps> = ({ currentStatus }) => {
  ...
  const fetchTodayData = async () => {
    try {
      ...
      if (employee && todayData) {
        const myRecord = todayData.find(record => 
    ...
    const data: ChartDataPoint[] = hours.map(hour => {
      ...
      todayAttendance.forEach(record => {
        ...
        if (record.check_in_time) {
          ...
          const endTime = record.check_out_time ? new Date(record.check_out_time) : currentTime
          
          if (checkIn.getHours() <= hour && endTime.getHours() >= hour) {
            const hoursInThisSlot = Math.min(1, (endTime.getTime() - checkIn.getTime()) / (1000 * 60 * 60))
      ...
      if (myTodayRecord) {
        if (myTodayRecord.check_in_time) {
          const myCheckInHour = new Date(myTodayRecord.check_in_time).getHours()
        ...
        if (myTodayRecord.check_out_time) {
          const myCheckOutHour = new Date(myTodayRecord.check_out_time).getHours()
  ...
  const calculateTotalHours = () => {
    if (myTodayRecord && myTodayRecord.check_in_time) {
      ...
      const checkOut = myTodayRecord.check_out_time ? new Date(myTodayRecord.check_out_time) : new Date()
        ...
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {`${entry.dataKey === 'clockIns' ? 'Clock Ins' : 
                 entry.dataKey === 'clockOuts' ? 'Clock Outs' : 
                 entry.dataKey === 'totalHours' ? 'Team Hours' : entry.dataKey}: ${entry.value}`}
            </p>
          ))}
        </div>
      )
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-900">Hours Today</p>
                    <p className="text-2xl font-bold text-blue-700">{totalHoursWorked}h</p>
                  </div>
                </div>
              </motion.div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-purple-700">
                      {currentStatus === 'working' ? 'Working' : 
                       ...
                       myTodayRecord?.check_in_time && !myTodayRecord?.check_out_time ? 'Checked In' :
                       'Off'}
                    </p>
                  </div>
                </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <ReferenceLine 
                    ...
                    stroke="#ef4444" 
                    strokeDasharray="4 4"
                    label={{ value: "Now", position: "topRight", offset: 10 }}
                  />
                  ...
                  {myTodayRecord && (
                    <>
                      ...
                      <Line 
                        ...
                        stroke="#dc2626" 
                        strokeWidth={3}
                        dot={{ fill: '#dc2626', r: 6, stroke: '#fff', strokeWidth: 2 }}
                        connectNulls={false}
                        name="My Clock Out"
                      />
                    </>
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 text-sm">
              <div className="flex items-center">
                ...
                <span className="text-gray-600">Team Clock Ins</span>
              </div>
              <div className="flex items-center">
                <div className="w-3 h-3 bg-yellow-500 rounded mr-2"></div>
                <span className="text-gray-600">Team Clock Outs</span>
              </div>
              <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <span className="text-gray-600">My Activity</span>
              </div>
              <div className="flex items-center">
